...
namespace noname.Controllers
{
    ...
    {
        readonly RoleService roleService = null;
        ...
        public async Task<ResponceModel> GetAll([FromQuery(Name = "text")] string text, [FromQuery(Name = "page")] int page = 1, [FromQuery(Name = "offset")] int offset = 20)
        {
            try
            {
                var roles = await roleService.GetAll(text, page, offset);
                return new ResponceModel(200, "OK", roles, null);
            }
            catch (Exception ex)
            {
                await _logService.Add(new SystemLog() { Content = ex.Message, CreateDate = DateTime.Now, UserId = 0, EntityName = roleService.GetType().Name });
        ...
        public async Task<ResponceModel> Get(int id)
        {
            try
            {
                ...
                return new ResponceModel(200, "OK", role, null);
            }
            catch (Exception ex)
            {
                await _logService.Add(new SystemLog() { Content = ex.Message, CreateDate = DateTime.Now, UserId = 0, EntityName = roleService.GetType().Name });
        ...
        [HttpPost]
        public async Task<ResponceModel> Add([FromBody] Role model)
        {
            ...
            try
            {
                var role = await roleService.Add(model);
                if (await roleService.SaveChanges())
                {
                    return new ResponceModel(200, "OK", role, null);
                }
                else
